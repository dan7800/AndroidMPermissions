...
public class GameActivity extends AppCompatActivity {
    ...
    private void initializeGame(){
       ...
       // vsComputer = getIntent().getStringExtra("GameType").equalsIgnoreCase("Computer")?true:false;
        ...
        if(vsComputer){
            setTitle("[23]TicTacToe - Play Against " + oponent);
            ...
            setTitle("[23]TicTacToe - Play Against Myself!");
        ...
        switch (requestCode) {
            case REQUEST_PERMISSION_CODE:
                for (int i = 0; i < permissions.length; i++) {
                    ...
                    // Permission required for "Share Score"
    ...
    private boolean shareScoreWithContacts() {
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.GET_ACCOUNTS) != PackageManager.PERMISSION_GRANTED) {
            if ((int) Build.VERSION.SDK_INT >= 23) {
                requestPermissions(new String[]{Manifest.permission.GET_ACCOUNTS}, REQUEST_PERMISSION_CODE);
            }
            return false;
        }
        return true;
